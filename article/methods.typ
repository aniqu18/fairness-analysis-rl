= Methods
This study deployed DQN and contextual bandits on the UCI Heart Disease-dataset // provide source of the dataset
to compare their predictive performance, bias and fairness between different patient groups. Both models can learn from sequential experiences, but they differ in how they balance exploration and exploitation and their approaches for policy optimization. The purpose of using the same dataset is to evaluate not only the overall predictive accuracy but also models perform with different categories and groups, particularly focusing on the sex-related differences. // age-related too @niko
...
Data collection is described in the #ref(<data_collection>, form: "normal"). Details about DQN model are provided in the #ref(<dqn>, form: "normal") and about contextual bandits in the #ref(<bandits>, form: "normal").

DQN and contextual bandits were trained on the train dataset. Then two iterations of evaluation were performed. The first one on the original dataset and the second one on the dataset resampled using SMOTE technique. 

During the first iteration for each model accuracy, F1-score, and recall were calculated with the respect to the different gender (male/female) and age (\<41, 41-50, 51-60, >60) groups on the test dataset.
...
Model performance was evaluated with accuracy and macro F1 score to give indication of predictive abilities. In addition, sex-based evaluation metrics and confusion matrices were computed to assess possible female and male differences. This analysis helped quantify both DQN-models predictive ability and its fairness, providing solid ground for comparison of other reinforcement learning model. // provide information about contextual bandits, age evaluation and recall. replace confusion matrices with sub-group analysis (?). add information about statistical testing @niko
